...
\textbf{Second  step}: if the reconstruction stencil is not across $x_{ii+\frac{1}{2}}$, we can directly use the high-order reconstruction method introduced in  Section \ref{hor}; If the stencil is across $x_{ii+\frac{1}{2}}$, we need define ghost cells. For example, we want to perform the third-order reconstruction on the the  left side of the interface and reconstruction needs two stencils: $S^1=(I_{ii-1},I_{ii}, I_{ii+1})$ and $S^2=(I_{ii},I_{ii+1}, I_{ii+2})$. We can see that cells $I_{ii+1}$ and $I_{ii+2}$ are on the other side of the interface with a different EOS. Directly using these two stencils to preform the reconstruction maybe result in numerical oscillations.
Shown in Fig.\ref{fig:ghost}, we need define two ghost cells $I'_{ii+1}$ and $I'_{ii+2}$ on the right side to replace $I_{ii+1}$ and $I_{ii+2}$.
...
The cell average values of the ghost cell $I'_{ii+1}$ and $I'_{ii+2}$ are given as follows: the Cauchy stress and the velocity  are  continuous across the interface according  to the relations in Eq.(\ref{e28}), so we use the real values on cells $I_{ii+1}$ and $I_{ii+2}$:
\begin{equation}
  \overline{\sigma}'_{ii+m} = \overline{\sigma}_{ii+m}, \quad \overline{u}'_{ii+m} = \overline{u}_{ii+m}, \ m=1,2.
\end{equation}
However, the values of pressure and density are not continuous, so we use a reflected condition on the interface as
\begin{equation}
\overline{p}'_{ii+m} = \overline{p}_{ii}, \quad \overline{\rho}'_{ii+m} = \overline{\rho}_{ii}, \ m=1,2.
\end{equation}

After we get all the primitive variables on  the ghost cells $I'_{ii+1}$ and $I'_{ii+2}$, using the EOS of material A on the left, we can get all the conservative variables  $\mathbf{Q}'_{ii+1}$ and $\mathbf{Q}'_{ii+2}$. 

\textbf{Third  step}: Define two new stencils $S^1=(I_{ii-1},I_{ii}, I'_{ii+1})$ and $S^2=(I_{ii},I'_{ii+1}, I'_{ii+2})$. Based on the new two stencils, use the high-order reconstruction method introduced in  Section \ref{hor} to perform the reconstruction on the left side of $x_{ii+\frac{1}{2}}$.
...
If we want to perform the reconstruction on the right side of $x_{ii+\frac{1}{2}}$, we use the similar process to do: define two new stencils $S^1=(I'_{ii-1},I'_{ii}, I_{ii+1})$ and $S^2=(I'_{ii},I_{ii+1}, I_{ii+2})$, variables on two ghost cells $I'_{ii-1}$ and $I'_{ii}$ are defines as follows:
\begin{equation*}
  \overline{\sigma}'_{ii+m} = \overline{\sigma}_{ii+m}, \quad \overline{u}'_{ii+m} = \overline{u}_{ii+m}, \ m=-1,0.
\end{equation*}
\begin{equation*}
\overline{p}'_{ii+m} = \overline{p}_{ii-1}, \quad \overline{\rho}'_{ii+m} = \overline{\rho}_{ii-1}, \ m=-1,0.
\end{equation*}
Other procedures are same as that for the reconstruction on the left side. 
