...
\end{equation}
%As $s^* > s_L$, we can also get a relation
...
\subsubsection{States behind the plastic wave}
Assume there exist a  plastic wave in the left side, the state in front of the wave is known,  according to Rankine-Hugoniot relation we can get 
\begin{equation}
  \widetilde{\rho}_L(\widetilde{u}_L-\widetilde{s}_L) = \rho_L(u_L-\widetilde{s}__L)\\
  \widetilde{\rho}\widetilde{u}_L(\widetilde{u}_L-\widetilde{s}_L) = \rho_Lu_L(u_L-\widetilde{s}__L)+\widetilde{\sigma}_L-\sigma_L\\
  \widetilde{\rho}\widetilde{E}_L(\widetilde{u}_L-\widetilde{s}_L) = \rho_LE_L(u_L-\widetilde{s}__L)+\widetilde{\sigma}_L-\sigma_Lu_L\\
...
\end{frame}
  ...
  the  we estimate the wave speeds of the elastic waves $s_L$, $s_R$ and $s^*$. The left going and right going wave speeds are estimate by the same strategy with HLLCE,
	\begin{equation}\label{eq:sLR}
	  s_L = \text{min} (u_L-c_L, u_R-c_R), \hspace{0.3cm} s_R = \text{max}(u_L+c_L, u_R+c_R).
	\end{equation}
	According to the Rankine-Hugoniot relations in regions $\bm{Q}_L$ and $\widetilde{\bm{Q}}_L$, 
