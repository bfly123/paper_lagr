...
r_4 = \frac{1}{\phi^2}\left[\begin{array}{l}
  ...
  \subsection{States between a  contact wave or interface}
  For an  equations system without molercular diffusion, there is always no materials across the contact wave, so the velocities between the discontinuity is always equal,$u_L^* = u_R^*$. This can also be verified by the eigenvectors (\ref{eq:eiv}), for $\lambda_1$-wave, 
  \begin{equation}
...
\begin{align}
  ...
  &\rho_R^* s^{*2}-\sigma^*_R=\rho_L^* u_L^{*2}-\sigma_L^*+s^*(\rho_R^* u_R^*-\rho_L^* u_L^*).
\end{align}
...
\subsubsection{Solving the Riemiann problem with only plastic waves}
At first we do not know the yielding states, so assume that there is no plastic wave in, we consider structures of Case.1 in Fig.1. If the result contains yielding process, we will turn the case to one in  Case.2-4  in Fig.1.
...
\begin{equation} \label{eq:rhoLstar}
  \rho_L^* u^*=\rho_L u_L+s_L(\rho_L^*-\rho_L),
\end{equation}
and
\begin{equation}
