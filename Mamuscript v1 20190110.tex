...
\color{red} With a high-order reconstruction method,  one stencil may across the interface and in a multi-material state, which will cause oscillation near the interface. In this paper, we use a ghost cell method in multi-material system to obtain a stable high-order reconstruction. \color{}  Combined with an improved third-order WENO scheme\cite{liu2018novel} and Runge-Kutta scheme, a high-order cell-centered Lagrangian scheme is given in this paper for one-dimensional multi-material elastic-plastic flows.
...
\end{figure}
 ...
 In the left, if  $|s_{xxL}| \le \frac{2}{3}Y_0 \le  |\hat{s}_{xxL}^*|$; but in the right, ($|s_{xxR}|<\frac{2}{3}Y_0$ and $|\hat{s}_{xxR}^*| < \frac{2}{3}Y_0$) or ($|s_{xxR}|\geq \frac{2}{3}Y_0$ and $|\hat{s}_{xxR}^*| \geq \frac{2}{3}Y_0$), there will be only one plastic wave which is in the left. Correspondingly, the Riemann structure of this case is showed in Fig.\ref{fig:case2}. We need to solve the yielding state of $\widetilde{\bm{Q}}_L$. The MHLLCEP is given as follows in this case,
...
\subsubsection {Case 3: with only the right plastic wave}\label{sec:case3}
 In the left, if  ($|s_{xxL}|<\frac{2}{3}Y_0$ and $|\hat{s}_{xxL}^*| < \frac{2}{3}Y_0$) or ($|s_{xxR}|\geq \frac{2}{3}Y_0$ and $|\hat{s}_{xxR}^*| \geq \frac{2}{3}Y_0$); but in the right, $|s_{xxR}| \le \frac{2}{3}Y_0 \le  |\hat{s}_{xxR}^*|$, there will be only one plastic wave which is in the right. Correspondingly, the Riemann structure of this case is showed in Fig.\ref{fig:case3}. We need to solve the yielding state of $\widetilde{\bm{Q}}_R$. The MHLLCEP is given as follows in this case,
